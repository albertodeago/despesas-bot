        ...
        for (const expense of recurrentExpenses) {
          ...
          if (now > lastAddedTs + 1000 * 60) {
            ...
            const subCategory = expense.subCategory
              ? ` - ${expense.subCategory}`
              : '';
            const message = `Spesa ricorrente aggiunta: ${expense.category}${subCategory} - ${expense.amount}€`;
